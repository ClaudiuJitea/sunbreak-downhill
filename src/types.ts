...
export interface World { obstacles?:Array<{s:number;lateral:number;radius:number}>; group: THREE.Group; length: number; totalLength?: number; sample(s: number, lateral?: number): TrackSample; height(x:number,z:number):number; update(camera:THREE.Camera, dt:number):void; checkpoints:number[]; trackId?:number; trackName?:string; trackSubtitle?:string; environment?:EnvironmentDef; jumpLips?: number[]; }
...
export interface RiderState { id:number; name:string; color:number; s:number; lateral:number; speed:number; y:number; vy:number; airborne:boolean; airTime:number; pitch:number; roll:number; yaw:number; lean:number; compression:number; cadence:number; crash:number; boost:number; score:number; trick:string; trickRotation:number; finished:boolean; finishTime:number; position:THREE.Vector3; velocity:THREE.Vector3; celebrate?:number; celebrateRank?:number; }
