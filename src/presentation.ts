...
export type CameraMode='title'|'chase'|'side'|'front'|'wide'|'replay'|'results';
...
export class Presentation {
  ...
  update(player:RiderState,dt:number,time:number,mode:CameraMode,boost:boolean,brake=false,wetness=0,enhanced=true){
    ...
    const blend=1-Math.exp(-dt*(mode==='title'?2.5:mode==='results'?3.2:5.8));
    ...
    const fov=mode==='title'?56:mode==='wide'?56:mode==='replay'?56:mode==='results'?54:59+speed*8+(boost?4:0);
